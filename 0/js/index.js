...
const giveMoney = function(){ // Inferred name => anonymus
    return true;
};

function getMoney(){ // Identifier name => cognitive
    return null;
};
...
// 1#
const power2 = (x) => { // curley brace and you must return output value in code
    return x * x;
};
...
// ** new ClassRoom creates a new Object and prototype links it to the existing ClassRoom prototype object meaning that MathClass has no access to all the prototype properties of ClassRoom;
...
function scheduleMeeting(start, durition){
    ...
    const meetingEndTime = `${String(endOfMeeting.hour)
        .padStart(2, "0")}:${String(endOfMeeting.minute)
        .padStart(2, "0")}`;
